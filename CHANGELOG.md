...
- **Sicherheits-Fix: `agent.info` Raw-Log entfernt (#58)**: `log.Printf("agent info from %s: %s", ...)` dumpte die komplette Message in die Dashboard-Logs. Entfernt — `handleAgentInfo` loggt die einzelnen relevanten Felder. Prophylaktisch gegen künftige sensible Felder.
- **Zwei Data-Races im Agent gefixt (#56)**:
  - **`Executor.OnProgress`**: Field wurde vom Read-Loop für die nächste Command geschrieben, während eine vorherige Execute-Goroutine es noch lesen konnte. Klassische Race auf Struct-Field. Field entfernt, jetzt per-call Parameter — kein shared mutable state.
  - **`LogStreamManager.streams`-Map**: Schreibzugriffe vom Caller und vom per-Stream Worker-Goroutine ohne Lock. Concurrent Map Writes panicen in Go zur Laufzeit. `sync.Mutex` rund um alle Operationen plus `fnEq`-Guard, damit der Worker beim Beenden nur seine eigene Map-Entry löscht (nicht eine von einem nachfolgenden `StartStream` geschriebene).
- **Heartbeat-Stalls eliminiert: Discovery in eigene Goroutine (#55)**: Bisher lief Docker-Discovery im selben Writer-Goroutine wie Heartbeats. Discovery kann bis zu 30 s Docker-Inspect/Stats blockieren — in der Zeit gehen Heartbeats nicht raus und das Dashboard markiert den Agent als offline obwohl er gesund ist, nur busy. Discovery jetzt auf eigenem Goroutine (`runDiscoveryLoop`), Ergebnisse über buffered Channel an den Writer.
- **WebSocket-Keepalive (#55)**: App-Level-Ping alle 25 s mit 10 s Timeout. Erkennt halb-offene Verbindungen (NAT-Idle-Timeout, stiller Intermediary-Drop) die TCP allein nicht bemerkt. Plus: Verbindungen die länger als 5 min hielten resetten den Reconnect-Backoff auf den Basiswert — späte Disconnects warten nicht mehr 60 s.
- **Tar-Traversal-Fix (#57)**: `extractTarGz` im Provisioner nutzte `filepath.Clean` + `strings.Contains(name, "..")` zum Validieren — fängt Edge-Cases nicht ab. Jetzt `filepath.Rel` zwischen absoluten Pfaden: Ziel muss strikt innerhalb des Destination-Verzeichnisses landen, sonst Skip.
- **`IsPortAvailable` mit Timeout (#57)**: Verwendete `Dial` ohne Timeout — bei firewalled Black-Hole-Ports (SYN silently dropped) blockte der Call indefinitely. `FindAvailablePort` ruft das in einer 100-Port-Loop auf, ein blockierter Port hätte das ganze Provisioning gestallt. `DialTimeout(500ms)` ergänzt.
- **`restart_container`-Field validiert (#57)**: `config.write`-Payload akzeptiert optional `restart_container`. Das Feld umging bisher die `containerNamePattern`-Validierung aus `ValidateCommand`. Jetzt mit derselben Regex geprüft, kann keinen unsafe Namen mehr durchschmuggeln.
- **Public-IP-Refresh (#57)**: Cached IP bei Agent-Start, nie aktualisiert. Bei DHCP-Renewal oder VPN-Flap blieb der Wert stale bis zum Agent-Restart. Jetzt `atomic.Value` mit eigenem Refresher-Goroutine alle 15 min, Heartbeats lesen per Getter-Closure. Log nur bei Change.
- **Restart-Drain (#57)**: Nach `agent.update`/`agent.restart` machte der Agent `time.Sleep(500ms)` und exec'd dann. Knapp — bei Last verlor das Dashboard das Command-Result. Jetzt `drainBeforeRestart` pollt die Result-/Progress-Channels (Empty-Detection + 200ms Settle, max 3 s) bis sie leer sind — Dashboard sieht zuverlässig das Resultat bevor das Re-Exec passiert.
- **Parallel Node-Polling (#59)**: `CollectAll` iterierte seriell über alle Nodes — eine unreachable Node blockte den ganzen Zyklus für 5 s, bei N down-Nodes wurde der Zyklus N×5 s lang. Jetzt parallel via WaitGroup + Mutex, jede Node hat ihr eigenes Timeout. Real-Timing-Test (5 Nodes × 100 ms, Limit 300 ms) beweist Parallelität. Slow-Poll-Warning ab `defaultNodePollTimeout`.
- **Discovery-Cadence 30 s → 60 s (#59)**: Discovery hat selbst bis zu 30 s Context-Timeout pro Zyklus — 30 s-Cadence ließ keinen Atemraum, back-to-back Discovery starvte andere Writes. 60 s ist immer noch ausreichend für die Node-Liste-Frische und entlastet den Writer.

### 2026-05-21 (Härtung Docker Self-Update, Follow-up zu #52)
- **`pullImage` erkennt jetzt Pull-Fehler**: Docker `/images/create` antwortet immer mit HTTP 200, Fehler (Auth, Manifest nicht gefunden, Transport) kommen als `errorDetail`-Zeilen im streamenden NDJSON-Body. Bisher haben wir den Stream verworfen und den Fehler erst indirekt beim folgenden `createContainer`-Call gesehen (mit unklarer Meldung). Jetzt parst `parseImagePullStream` jede Zeile und gibt einen klaren `pull failed: <Daemon-Message>` zurück.
- **Container-ID jetzt robust per `/proc/self/cgroup`**: `getSelfContainerID` liest die 64-stellige Container-ID aus dem cgroup-Pfad (funktioniert für cgroup v1 `/docker/<id>` und v2 `/system.slice/docker-<id>.scope`). Hostname bleibt Fallback, wird aber jetzt geprüft (`looksLikeContainerID`) statt blind übernommen — User mit `--hostname my-server` bekommen einen sauberen Fehler statt einem missverständlichen 404 von `inspectContainer`.
- **Startup-Janitor entfernt alte Finalize-Helper**: Beim Dashboard-Start scannt `SweepStaleFinalizeHelpers` nach Containern mit Name-Pattern `<self>-finalize-*` in nicht-laufendem State und entfernt sie. Bound auf Siblings des aktuellen Containers (per `inspectContainer(selfName)`). Best-effort, blockt den Start nicht. Schließt das einzige offene Loose End der `AutoRemove=false`-Politik aus PR #52: gescheiterte Updates hinterlassen ihre Helper-Logs für Diagnose, der nächste Restart räumt auf.
- **Tests**: `replaceImageTag` (inkl. Digest/Registry-Port-Edges), `parseImagePullStream` (success, errorDetail, error-field-only, mid-stream-fail, truncated-mid-object), `looksLikeContainerID`, `cgroupContainerIDRe` (cgroup v1 + v2 Format-Beispiele), `isContainerID`.
- **Audit-Härtung**: `looksLikeContainerID` akzeptiert nur lowercase Hex (Docker-IDs sind durchgehend lowercase — verhindert dass mixed-case Hostname-Strings als ID durchgehen), `parseImagePullStream` per `io.LimitReader` auf 64 MB begrenzt (defensiv gegen runaway-Daemon-Stream), `SweepStaleFinalizeHelpers` hat eine 30s-Deadline (hängende Daemon-Calls können den Startup sonst pro Helper bis zu 5 min stallen), Container-Filter im Janitor nutzt `json.Marshal` statt `fmt %q` (korrektes JSON-Escaping unabhängig vom Container-Namen).

### 2026-05-21
- **Fix: Docker Self-Update scheiterte an Port-Konflikt**: Der ursprüngliche Flow versuchte, den neuen Container zu starten, *bevor* der alte gestoppt war — Docker konnte den Host-Port (`9443`) nicht erneut binden und der Start schlug fehl (`Bind for 0.0.0.0:9443 failed: port is already allocated`). Rollback rannte, alter Container blieb auf der alten Version, Banner zeigte das Update weiterhin an. Bestätigt in Produktion (CTJaeger#50 Follow-up). Naive Lösung "alten erst stoppen, dann neuen starten" funktioniert nicht: die Goroutine, die den Update orchestriert, läuft *innerhalb* des alten Containers und wird beim Stop-Self mitgekillt — niemand startet dann den neuen Container.
  - **Neue Architektur**: kurzlebiger Sidecar-Container (Finalize-Helper), gestartet aus dem schon gepullten neuen Image mit `--self-update-finalize <new_id> --replaces <old_id>`. Mountet nur `/var/run/docker.sock`, keine Host-Ports. Flow: Old pullt → rename old → create new (im "Created"-Status, port noch frei) → start Helper → old stoppt sich selbst. Helper überlebt den Stop des alten Containers, wartet bis Old wirklich stopped ist, startet New (Port ist jetzt frei), entfernt Old, entfernt sich selbst.
  - **Neue CLI-Flags** (intern, nicht für User): `--self-update-finalize`, `--replaces`. Kurzschluss vor Daten-Dir/DB-Init in `cmd/dashboard/main.go`.
  - **Failure-Modes**: scheitert Helper-Start → kompletter Rollback (rename old back, remove new). Scheitert Helper *nach* Stop-Self → Old ist stopped-aber-renamed, New ist created-aber-not-started, Helper bleibt sichtbar (kein AutoRemove). User kann manuell starten: `docker start klever-node-hub`. Logs vom Helper bleiben für Diagnose erhalten.
  - **Härtung aus Code- + Security-Audit**: (a) Pro-Poll-Timeout (5s) in `waitContainerStopped` damit ein wedged Daemon nicht die 60s-Deadline überschreitet, (b) `replaceImageTag` weist Digest-Pins (`@sha256:...`) ab und respektiert Registry-Ports (`registry:5000/foo`), (c) Single-flight Mutex um `dockerSelfUpdate` — zwei parallele Klicks bekommen jetzt 409 statt sich zu verheddern, (d) `--self-update-finalize`/`--replaces` validieren Container-IDs (12-64 Hex, != self), (e) `stopContainer`-Fehler beim Self-Stop wird geloggt statt swallowed.
...
- **Reverse-Proxy-Setup-Guide**: Neue `docs/reverse-proxy.md` mit Schritt-für-Schritt-Anleitung für Apache, Nginx und Caddy inkl. Let's Encrypt — damit Browser die PWA installieren können (self-signed Cert blockt das sonst). Wichtig: Reverse-Proxy gehört auf einen Standalone-Host, Agents verbinden direkt auf Port 9443 am Proxy vorbei (mTLS).
- **Version Performance Regression**: Neuer RegressionDetector erkennt wenn eine neue Node-Version messbar langsamer ist. Vergleicht den Median von `klv_block_process_duration_ms` 24h vor vs. nach einem Versionswechsel — frühestens 12h nach dem Wechsel, Alarm nur bei +50% UND +30ms, danach `evaluated` → genau eine Warnung pro Versionswechsel, kein Spam. Migration 8: Tabelle `node_version_history`.
- **Passiver Performance-Report**: Node-Detail-Seite zeigt eine "Version Performance"-Card mit dem Block-Processing-Median vor/nach dem letzten Update als +/-%-Wert. Kein Alarm, nur Info. Neuer Endpoint `GET /api/nodes/{id}/performance`.
- **Heartbeat-Timeout-Setting verdrahtet**: Die Settings-Einstellung `heartbeat_timeout_sec` wurde gespeichert aber nie gelesen — zwei separate hardcoded 60s-Werte liefen stattdessen. Jetzt einzige Wahrheit: `SettingsStore.HeartbeatTimeout()` wird vom Hub-Health-Check (live bei jedem Tick) und der Agent-Offline-Alert-Rule genutzt.
- **Overview-Suchmodus**: Tippen in die Suche blendet Stat-Cards und Resources-Panel aus → Server/Node-Tabellen rutschen direkt unter das Suchfeld.
- **Sidebar-Alert-Badge global**: Roter Badge mit Anzahl aktiver Alerts auf jeder Seite, neuer "Tools"-Bereich mit Batch Config und Slot Inspector.
- **Fix: Doppeltes "Synced"**: `top-shell-status` wurde mit "Synced HH:MM" überschrieben — gibt's jetzt nur noch einmal im Header rechts.
- **Fix: Nonce ausgeschrieben**: Block-Nonce wird voll mit Tausender-Trennzeichen angezeigt (`30.500.000`) statt `30.5M` — in Snapshot, Status-Grid und Sync-Text.
